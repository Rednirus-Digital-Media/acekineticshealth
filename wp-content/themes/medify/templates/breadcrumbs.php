...
if (!class_exists('Medify_breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
     * @package medify\templates
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     * @version 1.1.2
     */
    class Medify_breadcrumbs
    {
        ...
        {
			$this->text_home     = esc_html__('Home', 'medify'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'medify'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'medify'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'medify'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'medify'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'medify'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'medify'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'medify'); // text 'Comment Page N'
        }
        ...
        public function __construct ()
        {
			...
			if (is_home() || is_front_page()) {
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(Medify::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep
                            . sprintf($link, $archive_link, esc_html__('Portfolio', 'medify'))
                            . $sep
                            . $before
				...
				// Custom Post Type
                    ...
                    && is_product()
                    && function_exists('medify_woocommerce_breadcrumb')
                ) {
	   					$args = [];
	   					$args['delimiter'] = $sep;
	   					if ($show_home_link) $out .= $sep;
	   					$out .= medify_woocommerce_breadcrumb($args);
	   			}
				$out .= $wrap_after;
            }

            echo Medify::render_html($out);
    	}
    }

    new Medify_breadcrumbs();
}
