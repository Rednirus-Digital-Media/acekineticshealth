...
if ( !(bool)$wgl_blog_atts ) {
    $opt_likes = Medify_Theme_Helper::get_option('blog_list_likes');
    $opt_share = Medify_Theme_Helper::get_option('blog_list_share');
    $opt_meta = Medify_Theme_Helper::get_option('blog_list_meta');
    $opt_meta_author = Medify_Theme_Helper::get_option('blog_list_meta_author');
    $opt_meta_comments = Medify_Theme_Helper::get_option('blog_list_meta_comments');
    $opt_meta_categories = Medify_Theme_Helper::get_option('blog_list_meta_categories');
    $opt_meta_date = Medify_Theme_Helper::get_option('blog_list_meta_date');
    $opt_read_more = Medify_Theme_Helper::get_option('blog_list_read_more');
    $opt_hide_media = Medify_Theme_Helper::get_option('blog_list_hide_media');
    $opt_hide_title = Medify_Theme_Helper::get_option('blog_list_hide_title');
    $opt_hide_content = Medify_Theme_Helper::get_option('blog_list_hide_content');
    $opt_letter_count = Medify_Theme_Helper::get_option('blog_list_letter_count');
    $opt_blog_columns = Medify_Theme_Helper::get_option('blog_list_columns');
    ...
    $wgl_blog_atts = array(
        ...
        'heading_tag' => 'h3',
        'read_more_text' => esc_html__('Read More', 'medify'),
        'items_load'  => 4,
...
while ($query->have_posts()) : $query->the_post();          
    ...
    <div class="blog-post <?php echo esc_attr($blog_item_classes); ?>"<?php echo Medify_Theme_Helper::render_html($blog_attr);?>>
        <div class="blog-post-hero_wrapper">
            ...
            <div class="blog-post-hero-content_front">
                ...
                <?php         
                    ...
                    wp_link_pages(array('before' => '<div class="page-link">' . esc_html__('Pages', 'medify') . ': ', 'after' => '</div>'));
                    ?>
                </div>
            </div>
            ...
            <?php
                ...
                ?>
                    <div class="btn-read-more">
                        <a href="<?php echo esc_url(get_permalink()); ?>" class="blog-post-read-more"></a> 
                    </div>
                 <?php
                endif;
            ?>
