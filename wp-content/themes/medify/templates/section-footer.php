...
if (!class_exists('Medify_footer_area')) {
	/**
	 * Footer area
	 *
	 *
	 * @class 		Medify_footer_area
	 * @version		1.0
	 * @category	Class
	 * @author 		WebGeniusLab
	 */
    ...
    class Medify_footer_area {
		...
		*/
    	...
    	function __construct () {
	        ...
	        $footer_bg_color = Medify_Theme_Helper::get_option('footer_bg_color');
	        // copyright option
	        $copyright_switch = Medify_Theme_Helper::get_option('copyright_switch');

			//add global variables
	        $this->footer_full_width = Medify_Theme_Helper::get_option('footer_full_width');
	        ...
	        $style .= Medify_Theme_Helper::bg_render('footer','mb_footer_switch','on');
	        $style = !empty($style) ? ' style="'.esc_attr($style).'"' : '' ;


	        //hide if 404 page
	        $page_not_found = Medify_Theme_Helper::get_option('404_show_footer');
			if(is_404() && !(bool) $page_not_found) $footer_switch = $copyright_switch = false;
    	...
    	private function get_wave_html(){
    		$wave_switch = Medify_Theme_Helper::options_compare('footer_add_wave','mb_footer_switch','on');
    		if((bool) $wave_switch){

    			$wave_height = Medify_Theme_Helper::get_option('footer_wave_height');
    			...
    			echo "<div class='medify_wave_footer' style='height:".(int) $wave_height."px;'>";
    	...
    	private function get_footer_vars($optn_1 = null){
    		...
    		//Get options
			$footer_spacing = Medify_Theme_Helper::options_compare('footer_spacing','mb_footer_switch','on');
			$footer_border = Medify_Theme_Helper::options_compare('footer_add_border','mb_footer_switch','on');
			$footer_border_color = Medify_Theme_Helper::options_compare('footer_border_color','mb_footer_switch','on');

	        // Only for widgets in footer
	        if ($optn_1 == 'widgets') {
				$footer_options['widget_columns'] = Medify_Theme_Helper::get_option('widget_columns');
		        $footer_options['widget_columns_2'] = Medify_Theme_Helper::get_option('widget_columns_2');
		        $footer_options['widget_columns_3'] = Medify_Theme_Helper::get_option('widget_columns_3');
		        $footer_align = Medify_Theme_Helper::get_option('footer_align');
	        ...
	        $footer_options['footer_border_style'] .= (bool)$footer_border ? ' style="border-top: 1px solid '.$footer_border_color.';"' : '' ;
		...
		private function main_footer_get_page()
		{
	        ...
	        echo "<div class='footer_top-area' ".$footer_border_style.">";
		...
		{
	        // Get options
    		$copyright_spacing = Medify_Theme_Helper::options_compare('copyright_spacing','mb_copyright_switch','on');
		...
		{
			$bg_color = Medify_Theme_Helper::options_compare('copyright_bg_color','mb_copyright_switch','on');
		...
		{
	        $editor = Medify_Theme_Helper::get_option('copyright_editor');

	        if ($this->mb_copyright_switch == 'on') {
	        	$editor = rwmb_meta('mb_copyright_editor');
	        }
	        ?>
    		<div class='copyright'<?php echo Medify_Theme_Helper::render_html($this->copyright_style()); ?> >
                <?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
                	<div class='row' <?php echo Medify_Theme_Helper::render_html($this->copyright_spacing());?> >
