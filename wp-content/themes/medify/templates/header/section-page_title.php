...
*
*
* @class		Medify_get_page_title
* @version		1.0
* @category		Class
* @author		WebGeniusLab
*/

if (!class_exists('Medify_get_page_title')) {
	class Medify_get_page_title{
		...
		public function init() {
			...
			$this->page_title_switch = Medify_Theme_Helper::get_option('page_title_switch') == '1' || Medify_Theme_Helper::get_option('page_title_switch') == true ? 'on' : 'off';
		...
		private function check_single_type() {
			if (get_post_type(get_queried_object_id()) == 'post' && is_single()){
				$single_type = Medify_Theme_Helper::get_option('single_type_layout');
	   	...
	   	public function page_title_render_html() {
			...
			$page_title_font = Medify_Theme_Helper::options_compare('page_title_font', 'mb_page_title_switch', 'on');
			$page_title_breadcrumbs_font = Medify_Theme_Helper::options_compare('page_title_breadcrumbs_font', 'mb_page_title_switch', 'on');
			$page_title_breadcrumbs_switch = Medify_Theme_Helper::options_compare('page_title_breadcrumbs_switch', 'mb_page_title_switch', 'on');
			$page_title_parallax = Medify_Theme_Helper::options_compare('page_title_parallax', 'mb_page_title_switch', 'on');
			$page_title_parallax_speed = apply_filters("pagetitle_parallax_speed", Medify_Theme_Helper::options_compare('page_title_parallax_speed', 'mb_page_title_switch', 'on'));
			...
			$medify_page_title = $this->medify_page_title();

			if (is_home() || is_front_page()) {
				$this->page_title_switch = 'off';
			}

			$page_not_found = Medify_Theme_Helper::get_option('404_page_title_switcher');
	        ...
	        if ($this->page_title_switch == 'on') {
					...
					$output .= '<div class="page-header_wrapper">';
						$output .= "<div class='wgl-container'>";
							$output .= "<div class='page-header_content'>";
								...
								if(function_exists('is_product') && is_product()){
					   				if(function_exists('medify_woocommerce_prev_next')){
					   					$output .= medify_woocommerce_prev_next();
					   				}
								}
								if(function_exists('is_product') && is_product()){
									$output .= "</div>";
				...
				echo Medify_Theme_Helper::render_html($output);
	        }
	   	}

		public function medify_page_title() {
			$title = '';
			if (is_home() || is_front_page()) {
				$title = '';
			} elseif ( is_category() ) {
				...
				$title = single_term_title("", false).esc_html__( ' Tag', 'medify' );
			} elseif ( is_date() ) {
				$title = get_the_time('F Y');
			} elseif( is_author() ) {
				$title = esc_html__( 'Author:', 'medify' ) .' '. get_the_author();
			} elseif ( is_search() ) {
				$title = esc_html__( 'Search', 'medify' );
			} elseif ( is_404() ) {
				$title = (bool)Medify_Theme_Helper::get_option('404_custom_title_switch') ? Medify_Theme_Helper::get_option('404_page_title_text') : esc_html__( 'Error Page', 'medify' );
			} elseif ( is_archive() ) {
				...
				$portfolio_title_conditional = Medify_Theme_Helper::get_option('portfolio_title_conditional') == '1' ? 'on' : 'off';
	            $portfolio_title_text = !empty(Medify_Theme_Helper::get_option('portfolio_single_page_title_text')) ? Medify_Theme_Helper::get_option('portfolio_single_page_title_text') : '';

				$title = $portfolio_title_conditional == 'on' ? esc_html($portfolio_title_text) : esc_html(get_the_title());
				$title = apply_filters('medify_page_title_portfolio_text', $title);

	        } elseif ( is_singular('team') ) {

				$team_title_conditional = Medify_Theme_Helper::get_option('team_title_conditional') == '1' ? 'on' : 'off';
	            $team_title_text = !empty(Medify_Theme_Helper::get_option('team_single_page_title_text')) ? Medify_Theme_Helper::get_option('team_single_page_title_text') : '';

				$title = $team_title_conditional == 'on' ? esc_html($team_title_text) : esc_html(get_the_title());
				$title = apply_filters('medify_page_title_team_text', $title);

	        } elseif ( function_exists('is_product') && ( is_product() ) ) {
	        	$shop_title_conditional = Medify_Theme_Helper::get_option('shop_title_conditional') == '1' ? 'on' : 'off';
	            $shop_title_text = !empty(Medify_Theme_Helper::get_option('shop_single_page_title_text')) ? Medify_Theme_Helper::get_option('shop_single_page_title_text') : '';

				$title = $shop_title_conditional == 'on' ? esc_html($shop_title_text) : esc_html(get_the_title());
				$title = apply_filters('medify_page_title_shop_text', $title);
	        }
	        else {
	            global $post;
	            ...
	            if (!empty($post)) {
	                ...
	                $blog_title_conditional = Medify_Theme_Helper::get_option('blog_title_conditional') == '1' ? 'on' : 'off';
	                $blog_title_text = !empty(Medify_Theme_Helper::get_option('post_single_page_title_text')) ? Medify_Theme_Helper::get_option('post_single_page_title_text') : '';

	                if ($posttype == 'post') {
						$title = $blog_title_conditional == 'on' ? esc_html($blog_title_text) : esc_html(get_the_title($id));
						$title = apply_filters( 'medify_page_title_blog_text', $title );
	                } else {
	                ...
	                $title = esc_html__('No Posts','medify');
	            }
	        ...
	        if ($this->mb_page_title_switch == 'on') {
	        	...
	        	if(!empty($custom_title_switch)){
	        		...
	        		$title = apply_filters( 'medify_page_title_custom_text', $title );
	        	}
	        }
	   	...
	   	public function page_title_classes() {
			...
			if ( is_singular('portfolio') || function_exists('is_product') && is_product() ) { // Portfolio single, Shop single have individual options for fine customization
				...
				$page_title_align = Medify_Theme_Helper::get_option($post_type.'_single_title_align');
				$breadcrumbs_align = Medify_Theme_Helper::get_option($post_type.'_single_breadcrumbs_align');
				$breadcrumbs_block = Medify_Theme_Helper::get_option($post_type.'_single_breadcrumbs_block_switch');
				...
				$page_title_align = Medify_Theme_Helper::options_compare( 'page_title_align', 'mb_page_title_switch', 'on' );
				$breadcrumbs_align = Medify_Theme_Helper::options_compare( 'page_title_breadcrumbs_align', 'mb_page_title_switch', 'on' );
				$breadcrumbs_block = Medify_Theme_Helper::get_option('page_title_breadcrumbs_block_switch');
			}
	   	...
	   	public function page_title_styles() {
			$page_title_bg_switch = Medify_Theme_Helper::options_compare( 'page_title_bg_switch', 'mb_page_title_switch', 'on' );
			$page_title_margin = Medify_Theme_Helper::options_compare( 'page_title_margin', 'mb_page_title_switch', 'on' );
			$page_title_padding = Medify_Theme_Helper::options_compare( 'page_title_padding', 'mb_page_title_switch', 'on' );
			$page_title_border_top_switch = false;
			$page_title_border_color = '';

			if ($this->page_title_switch == 'on') {
				$page_title_bg_color = Medify_Theme_Helper::get_option('page_title_bg_image')['background-color'];
				$page_title_height = Medify_Theme_Helper::get_option('page_title_height')['height'];
			}
			...
			if ( is_singular('portfolio') || function_exists('is_product') && is_product() || is_404() ) {
				...
				$page_title_bg_switch = Medify_Theme_Helper::get_option($post_type.'_title_bg_switch');
				$page_title_bg_color = Medify_Theme_Helper::get_option($post_type.'_page_title_bg_image')['background-color'];
				$page_title_margin = Medify_Theme_Helper::get_option($post_type.'_page_title_margin');
				$page_title_padding = Medify_Theme_Helper::get_option($post_type.'_page_title_padding');
				if ( class_exists('RWMB_Loader') && ($this->id !== 0) && (rwmb_meta('mb_page_title_switch') == 'on') ) {
				...
				if ($post_type == 'shop_single') {
					$page_title_border_top_switch = Medify_Theme_Helper::get_option($post_type.'_page_title_border_switch');
					$page_title_border_color = Medify_Theme_Helper::get_option($post_type.'_page_title_border_color')['rgba'];
					if ( class_exists('RWMB_Loader') && ($this->id !== 0) && (rwmb_meta('mb_page_title_switch') == 'on') ) {
						$page_title_border_top_switch = rwmb_meta('mb_page_title_border_switch');
						$page_title_border_color = rwmb_meta('mb_page_title_border_color');
					}
			...
			if ( is_404() ) {
				switch ( (bool)$page_title_bg_switch ) {
					case true:
						$style .= !empty(Medify_Theme_Helper::bg_render('404_page_title')) ? Medify_Theme_Helper::bg_render('404_page_title') : Medify_Theme_Helper::bg_render('page_title');
						break;
					default: break;
				}
			} elseif ( (bool)$shop_title && !empty(Medify_Theme_Helper::bg_render('shop_'.$shop_title.'_page_title')) ) {
				$style .= function_exists('is_product') && !is_product() ? Medify_Theme_Helper::bg_render('shop_'.$shop_title.'_page_title') : ((bool)$page_title_bg_switch ? Medify_Theme_Helper::bg_render('shop_single_page_title') : '');
			} elseif ( (bool)$cpt_title && (bool)$page_title_bg_switch && !empty(Medify_Theme_Helper::bg_render($cpt_type_title.'_'.$cpt_title.'_page_title')) ) {
				$style .= Medify_Theme_Helper::bg_render($cpt_type_title.'_'.$cpt_title.'_page_title');
			} else {
				$style .= (bool)$page_title_bg_switch ? Medify_Theme_Helper::bg_render('page_title','mb_page_title_switch','on') : '';
			}
