...
use Elementor\Utils;
use Medify_Theme_Helper as Medify;

if (!class_exists('Medify_get_header')) {
	class Medify_get_header
	{
		...
		public function header_vars()
		{
	    	...
	    	$this->name_preset = Medify::get_option('header_def_js_preset');
	    	$get_def_name = get_option('medify_set_preset');
	    	if (!$this->in_array_r($this->name_preset, get_option('medify_set_preset'))) {
	    	...
	    	if (
                ...
                if (!$this->in_array_r(rwmb_meta('mb_customize_header'), get_option( 'medify_set_preset' ))) {
                } else {
                    $get_def_name = get_option('medify_set_preset');
		...
		public function init()
		{
	        ...
	        //hide if 404 page
	        $page_not_found = Medify::get_option('404_show_header');
		...
		*/
           ...
           {
	   		$header_shadow = Medify::get_option( 'header_shadow',$this->name_preset, $this->def_preset );
	   		$header_on_bg = Medify::get_option( 'header_on_bg',$this->name_preset, $this->def_preset );
		...
		*/
        public function header_bar_editor($location = null, $position = null)
        {
			...
			*/
			...
			${'header_'.$position.'_editor'} = Medify::get_option($location.'_header_bar_'.$position.'_editor', $this->name_preset, $this->def_preset);
		...
		*/
        public function header_bar_delimiter($k = null)
        {
			...
			$header_sticky_builder = Medify::get_option('sticky_header');
				...
				*/
				...
				$height = Medify::get_option( 'bottom_header_delimiter'.$get_number.'_height', $this->name_preset, $this->def_preset );
				$width = Medify::get_option( 'bottom_header_delimiter'.$get_number.'_width', $this->name_preset, $this->def_preset );

				$bg_color = Medify::get_option( 'bottom_header_delimiter'.$get_number.'_bg', $this->name_preset, $this->def_preset );

				$margin = Medify::get_option( 'bottom_header_delimiter'.$get_number.'_margin', $this->name_preset, $this->def_preset );
		        ...
		        if ( $this->html_render === 'sticky' ) {
		        	$custom_sticky = Medify::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom', $this->name_preset, $this->def_preset);
		        	if( !empty($custom_sticky) ){
		        		$bg_color = Medify::get_option('bottom_header_delimiter'.$get_number.'_sticky_color', $this->name_preset, $this->def_preset);
		        		$height  = Medify::get_option('bottom_header_delimiter'.$get_number.'_sticky_height', $this->name_preset, $this->def_preset);
		        	}
		        }
		...
		*/
        public function header_bar_button($k = null)
        {
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = Medify::get_option('bottom_header_button'.$get_number.'_title', $this->name_preset, $this->def_preset);

			$link = Medify::get_option('bottom_header_button'.$get_number.'_link', $this->name_preset, $this->def_preset);

			$target = Medify::get_option('bottom_header_button'.$get_number.'_target', $this->name_preset, $this->def_preset);

			$size = Medify::get_option('bottom_header_button'.$get_number.'_size', $this->name_preset, $this->def_preset);

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = Medify::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn, $this->name_preset, $this->def_preset);

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = Medify::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn, $this->name_preset, $this->def_preset);

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = Medify::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn, $this->name_preset, $this->def_preset);

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = Medify::get_option('bottom_header_button'.$get_number.'_border'.$options_btn, $this->name_preset, $this->def_preset);
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = Medify::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = Medify::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = Medify::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn, $this->name_preset, $this->def_preset);
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';

			$border_radius = Medify::get_option('bottom_header_button'.$get_number.'_radius', $this->name_preset, $this->def_preset);
			switch ($size) {
				case 's':
					$size = 'sm';
					break;
			...
			$button_css_id =  uniqid( "medify_button_" );
	        ...
	        ob_start();
				...
				if(isset($border_radius)){
					echo "#$button_css_id {
						border-radius: ".( (int) esc_attr($border_radius) !== 0  ? (int) esc_attr($border_radius).'px' : '0px').";
					}";
				}
    		...
    		echo '<div class="header_button">';
    			echo '<div class="wrapper">';
		            ...
		            ?>
		            <div <?php echo Medify::render_html($this->get_render_attribute_string( 'wrapper' )); ?>>
		                <a <?php echo Medify::render_html($this->get_render_attribute_string( 'button' )); ?>>
        ...
        public function render_text($settings)
        {
            ...
            ?>
            <span <?php echo Medify::render_html($this->get_render_attribute_string( 'content-wrapper' )); ?>>
                <span <?php echo Medify::render_html($this->get_render_attribute_string( 'text' )); ?>><?php echo Medify::render_html($settings['text']); ?></span>
            </span>
            <?php
        }
		...
		*/
        public function header_bar_spacer($location = null, $key = null)
        {
			...
			*/
			...
			$spacer = Medify::get_option($location.'_header_spacer'.$get_number, $this->name_preset, $this->def_preset);
		...
		*/
	   	public function build_header_layout($section = 'bottom'){
	   		...
	   		$header_layout = Medify::get_option($section.'_header_layout', $this->name_preset, $this->def_preset);
	   		$lavalamp_active = Medify::get_option('lavalamp_active', $this->name_preset, $this->def_preset);
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = Medify::get_option('header'.$s.'_full_width',$this->name_preset, $this->def_preset);
				...
				case 'sticky':
					$width_container = Medify::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
			   			$class = "fullwidth-wrapper";
			   		} else {
			   			$class = 'wgl-container';
			...
			* @access private
			*/
			$header_background = Medify::get_option('header'.$s.'_background', $this->name_preset, $this->def_preset);
			$header_background_image = Medify::get_option('header'.$s.'_background_image', $this->name_preset, $this->def_preset);
			$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';

			$header_color = Medify::get_option('header'.$s.'_color', $this->name_preset, $this->def_preset);
			$header_bottom_border = Medify::get_option('header'.$s.'_bottom_border', $this->name_preset, $this->def_preset);
			$header_border_height = Medify::get_option('header'.$s.'_border_height', $this->name_preset, $this->def_preset);
			$header_border_height = $header_border_height['height'];
			$header_bottom_border_color = Medify::get_option('header'.$s.'_bottom_border_color', $this->name_preset, $this->def_preset);
			...
			*/
			...
			$header_sticky_height = Medify::get_option('header_sticky_height');
	   		$header_mobile_height = Medify::get_option('header_mobile_height');

	   		$header_height = Medify::get_option('header'.$s.'_height', $this->name_preset, $this->def_preset);
	   	...
	   	private function side_panel_style_icon($s = '_middle'){
	   		...
	   		$sticky_icon_switcher = Medify::get_option('bottom_header_side_panel_sticky_custom', $this->name_preset, $this->def_preset);
			...
			$icon_background = Medify::get_option('bottom_header_side_panel'.$value.'_background', $this->name_preset, $this->def_preset);
			$icon_color = Medify::get_option('bottom_header_side_panel'.$value.'_color', $this->name_preset, $this->def_preset);
			...
			* @access private
			*/
			$dispay = Medify::get_option('header_column'.$s.'_'.$area.'_display', $this->name_preset, $this->def_preset);
			$v_align = Medify::get_option('header_column'.$s.'_'.$area.'_vert', $this->name_preset, $this->def_preset);
			$h_align = Medify::get_option('header_column'.$s.'_'.$area.'_horz', $this->name_preset, $this->def_preset);
		...
		*/
           public function build_header_mobile_menu($preset = null, $def_preset = null)
           {
	   		...
	   		$header_queris = Medify::get_option('header_mobile_queris', $preset, $def_preset);
	   		$sub_menu_position = Medify::get_option('mobile_position');
                ...
                echo "<div class='wgl-menu_outer" . (!empty($sub_menu_position) ? ' sub-menu-position_' . esc_attr($sub_menu_position) : '') . "' id='wgl-perfect-container'>";
                    echo "<nav class='primary-nav'>";
                        ...
                        echo '</div>';
			   			...
			   			$menu = '';
			   			$custom_mobile_menu = Medify::get_option('custom_mobile_menu');

						if (!empty($custom_mobile_menu)) {
							$menu = Medify::get_option('mobile_menu');
						}
        ...
        {
	    	$mobile_header_custom = Medify::get_option('mobile_header');
        ...
        */
        public static function get_logo($location, $menu = false)
        {
            ...
            $header_logo_src = Medify::get_option('header_logo');
            ...
            // Get Sticky Logotype
            $logo_sticky_src = Medify::get_option('logo_sticky');
            ...
            $logo_mobile_src =  Medify::get_option('logo_mobile' . $menu);
            ...
            $logo_height_custom = Medify::get_option('logo_height_custom');
            $logo_height = Medify::get_option('logo_height');
            $logo_height = $logo_height['height'];


            $sticky_logo_height_custom = Medify::get_option('sticky_logo_height_custom');
            $sticky_logo_height = Medify::get_option('sticky_logo_height');
            $sticky_logo_height = $sticky_logo_height['height'];

            $mobile_logo_height_custom = Medify::get_option('mobile_logo'.$menu.'_height_custom');
            $mobile_logo_height = Medify::get_option('mobile_logo'.$menu.'_height');
			...
			switch (true) {
				case $location == 'bottom':
					if (!empty($header_logo_src)) {
						?>
						<img class="default_logo" src="<?php echo esc_url($header_logo_src); ?>" alt="<?php echo esc_attr($def_img_alt); ?>" <?php echo Medify::render_html($logo_height_style);?>>
					<?php
					} else {
						?>
						<h1 class="logo-name">
				...
				case !empty($logo_sticky_src) && $location == 'sticky':
					?>
					<img class="logo-sticky" src="<?php echo esc_url($logo_sticky_src);?>" alt="<?php echo esc_attr($sticky_img_alt); ?>" <?php echo Medify::render_html($sticky_height_style);?>>
					<?php
					break;

				case !empty($logo_mobile_src) && $location == 'mobile':
					?>
					<img class="logo-mobile" src="<?php echo esc_url($logo_mobile_src);?>" alt="<?php echo esc_attr($mobile_img_alt); ?>" <?php echo Medify::render_html($mobile_height_style);?>>
					<?php
					break;
		...
		*/
	    public function search($html_render = '',$location = ''){
	    	$description = esc_html__('Type To Search', 'medify');
			$search_style = Medify::get_option('search_style');
	        ...
	        $search_class = ' search_'.Medify::get_option('search_style');
					...
					$output .= '<div class="header_search-field">';
						if ($search_style === 'alt') {
							$output .= '<div class="header_search-wrap">';
								$output .= '<div class="medify_module_double_headings aleft">';
								$output .= '<h3 class="header_search-heading_description heading_title">'.apply_filters('medify_desc_search', $description) .'</h3>';
								$output .= '</div>';
							$output .= '</div>';
	    ...
	    public function login_in($location, $section){
	        ...
	        echo "<div class='login-in woocommerce'".$this->row_style_height($location).">";
	        	...
	        	if ( is_user_logged_in() ){
	        		...
	        		echo "<a class='login-in_link-logout' href='".esc_url( $link_logout )."'>".esc_html__('Logout', 'medify')."</a>";
	        	}else{
	        		echo "<a class='login-in_link' href='".esc_url_raw( $url )."'>".esc_html__('Login', 'medify')."</a>";
	        	}
		...
		*/
		...
		public static function icon_cart() {
			...
			?>
				<a class="woo_icon" title="<?php esc_attr_e( 'Click to open Shopping Cart', 'medify' ); ?>">
