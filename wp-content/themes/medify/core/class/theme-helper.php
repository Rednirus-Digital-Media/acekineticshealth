...
if (!class_exists('Medify_Theme_Helper')) {
    ...
    class Medify_Theme_Helper
    {
        ...
        public static function get_option($name, $preset = null, $def_preset = null) {
            if (  class_exists( 'Redux' ) && class_exists( 'Medify_Core_Public' ) ) {
                $preset = $preset == 'default' ? null : $preset;

                if (!$preset) {

                    // Customizer
                    if (!empty($GLOBALS['medify_set']) && $GLOBALS['medify_set'] != NULL) {
                        $theme_options = $GLOBALS['medify_set'];
                    } else {
                        $theme_options = get_option( 'medify_set' );
                    }

                } else {
                    $theme_options = get_option( 'medify_set_preset' );
                }

                if (empty($theme_options)) {
                    $theme_options = get_option( 'medify_default_options' );
                }
                ...
                $default_option = get_option( 'medify_default_options' );
		...
		public static function load_more($query = false, $name_load_more = '', $class = ''){
			$name_load_more = !empty($name_load_more) ? $name_load_more : esc_html__( 'Load More', 'medify' );
        ...
        public static function header_preset_name(){
            ...
            if( !self::in_array_r($name_preset, get_option( 'medify_set_preset' ))){
                $name_preset = 'default';
            }
        ...
        public static function render_sidebars($args = 'page'){
            ...
            $medify_core = class_exists('Medify_Core');
        ...
        public static function posted_meta_on(){
            global $post;
            $text_string = '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'medify') . ' <a href="%3$s" rel="gallery">%4$s</a></span>';
            ...
            if ( $metadata ) {
                printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s" title="%2$s">%1$s %3$s &times; %4$s</a></span>',
                    esc_html_x( 'Full size', 'Used before full size attachment link.', 'medify' ),
            ...
            edit_post_link(
                sprintf(
                    ...
                    wp_kses( __( 'Edit<span class="screen-reader-text"> "%s"</span>', 'medify' ), $allowed_html ) ,
                        get_the_title()
