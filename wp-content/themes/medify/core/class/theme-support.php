...
/**
* Medify Theme Support
*
*
* @class        Medify_Theme_Support
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('Medify_Theme_Support')) {
    class Medify_Theme_Support{
        ...
        public function register_my_menus(){
            register_nav_menus(
                array(
                    'main_menu' => esc_html__('Main menu', 'medify')
                )
            );
        }        

        public function enqueue_translation_files(){
            load_theme_textdomain('medify', get_template_directory() . '/languages/');
        }

        public function sidebar_register(){
            // Get List of registered sidebar
            $custom_sidebars = Medify_Theme_Helper::get_option('sidebars');

            // Default wrapper for widget and title
            $wrapper_before = '<div id="%1$s" class="widget medify_widget %2$s">';
            $wrapper_after = '</div>';
            ...
            if (!empty($custom_sidebars)) {
                foreach($custom_sidebars as $single){  
                    register_sidebar( array(  
                        ...
                        'description' => esc_html__('Add widget here to appear it in custom sidebar.', 'medify'),
            ...
            $footer_columns = array(
                array(
                    'name' => esc_html__( 'Footer Column 1', 'medify' ),
                    'id' => 'footer_column_1'
                ),
                array(
                    'name' => esc_html__( 'Footer Column 2', 'medify' ),
                    'id' => 'footer_column_2'
                ),
                array(
                    'name' => esc_html__( 'Footer Column 3', 'medify' ),
                    'id' => 'footer_column_3'
                ),
                array(
                    'name' => esc_html__( 'Footer Column 4', 'medify' ),
                    'id' => 'footer_column_4'
                ),
            );
            ...
            foreach ($footer_columns as $key => $footer_column) {
                register_sidebar( array(
                    ...
                    'id'            => $footer_column['id'],
                    'description' => esc_html__('This area will display in footer like a column. Add widget here to appear it in footer column.', 'medify'),
                    'before_widget' => $wrapper_before,  
                    'after_widget' => $wrapper_after,
                    'before_title' => $title_before,
                    'after_title' => $title_after,
            ...
            if (class_exists( 'WooCommerce' )){
                $shop_sidebars = array(
                    array(
                        'name' => esc_html__( 'Shop Products', 'medify' ),
                        'id' => 'shop_products'
                    ),
                    array(
                        'name' => esc_html__( 'Shop Single', 'medify' ),
                        'id' => 'shop_single'
                    ),                    
                    array(
                        'name' => esc_html__( 'Shop Filter', 'medify' ),
                        'id' => 'shop_filter'
                    )
                );
                foreach ($shop_sidebars as $key => $shop_sidebar) {
                    register_sidebar( array(
                        ...
                        'id'            => $shop_sidebar['id'],
                        'description' => esc_html__('This sidebar will display in WooCommerce Pages.', 'medify'),
                        'before_widget' => $wrapper_before,  
                        'after_widget' => $wrapper_after,
                        'before_title' => $title_before,
                        'after_title' => $title_after,
