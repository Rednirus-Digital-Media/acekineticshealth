...
*
*
* @class        Medify_Theme_Helper
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('Medify_Single_Post')) {

	class Medify_SinglePost{
		/**
	     * @var SinglePost
	     */
	    ...
	    public function videobox_hero(){
	    	...
	    	$output .= '<div class="wgl-video_popup button_align-center animation_circles">';
	    ...
	    public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null, $show_media = true ) {
			...
			$featured_image = Medify_Theme_Helper::options_compare('featured_image_type', 'mb_featured_image_conditional', 'custom');
			if ($featured_image == 'replace') {
				$featured_image_replace = Medify_Theme_Helper::options_compare('featured_image_replace', 'mb_featured_image_conditional', 'custom');
			}
	    	...
	    	if((bool) $media && (bool) $show_media){
	    		if($this->post_format === 'video' && $video_style == 'bg_video'){
	    			echo Medify_Theme_Helper::render_html($media);
			...
			<?php
           	...
           	?>
           		<div class="blog-post_quote-author">
           			<?php
           			echo Medify_Theme_Helper::render_html($autor_avatar);
	    	...
	    	if($this->show_author_meta || $this->show_comments_meta || $this->show_date_meta || $this->show_category_meta){
		    	...
		    	<div class="meta-wrapper">
					...
					<?php if($this->show_comments_meta) :
						...
						$comments_text = '' . $comments_num == 1 ? esc_html__('Comment', 'medify') : esc_html__('Comments', 'medify')  . '';
					?>
	    ...
	    public function render_excerpt ($symbol_count = false, $shortcode = false, $read_more = false, $read_more_text = false) {
			...
			if ((bool)Medify_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = Medify_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
	    ...
	    public function get_post_views($postID){
		    ...
		    if($count==''){
		    	...
		    	echo '<span class="counts_text">'. esc_html__(' View', 'medify'). '</span>';
		    }else{
				echo '<span class="counts">'. esc_html($count). '</span>';
		    	echo '<span class="counts_text">'. esc_html__(' Views', 'medify'). '</span>';
		    }
			echo '</div>';
	    ...
	    public function render_author_info () {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">'.esc_html__('About', 'medify').'</span>'.$user_first.' '.$user_last.'</h5>' : '';
		...
		public function get_render_attribute_string($element)
		{
			...
			return ' ' . Medify_Theme_Helper::render_html_attributes($this->render_attributes[$element]);
		}
	}
