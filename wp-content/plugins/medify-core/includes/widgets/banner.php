...
private $isMultilingual = FALSE; //Is this site multilingual?
    ...
    function __construct() 
    {
        parent::__construct(
            ...
            esc_html__( 'WGL Banner', 'medify-core' ), // Name
            array( 'description' => esc_html__( 'WGL Widget', 'medify-core' ), ) // Args
        );
    ...
    public function enqueue_backend_scripts()
    {
        ...
        wp_enqueue_script('medify-upload-img', get_template_directory_uri() . '/core/admin/js/img_upload.js', array('jquery'), false, true);
    }
    ...
    public function widget( $args, $instance ) 
    {
        ...
        if ('' === get_post_meta($attachment_id, '_wp_attachment_image_alt', true)) {
            $alt = the_title_attribute(array('before' => esc_html__('Featured image: ', 'medify-core'), 'echo' => false));
        } else {
        ...
        $widgetClasses = 'medify_banner-widget';
        $widgetClasses.= ' widget medify_widget';
        ...
        if ( !empty($button_link) && empty($widgetImg) && empty($title) ) {
            ...
            $html .= '<div class="medify_banner-widget-wrapper">';
                $html .= '<div class="medify_banner-widget-wrapper-inner" style="'.esc_attr($spacing_box).'">';
                    if ( !empty($widgetImg) ) {
                    ...
                    if ( !empty($text) ) {
                        $html .= '<h2 class="banner-widget_text">' . Medify_Theme_Helper::render_html($text);
                        $html .= '</h2>';
                    } 

                    if ( !empty($text_sub_title) ) $html .= '<p class="banner-widget_text_sub">' . Medify_Theme_Helper::render_html($text_sub_title) . '</p>';
        ...
        echo Medify_Theme_Helper::render_html($html);
    }
    ...
    public function form( $instance ) {
        ...
        $defaults = array(
            'padding_top_value'     => '38',
            'padding_bottom_value'  => '46',
        );
        ...
        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>"><?php echo esc_html__('Add Logotype image:', 'medify-core')?></label><br />
            <img class="medify_media_image" src="<?php if(!empty($instance[$image_name])){echo esc_url( $instance[$image_name] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat medify_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>" value="<?php echo esc_attr( $image ); ?>">
            <a href="#" class="button medify_media_upload"><?php esc_html_e('Upload', 'medify-core'); ?></a>
        </p>

        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>"><?php echo esc_html__('Add background image:', 'medify-core')?></label><br />
            <img class="medify_media_image" src="<?php if(!empty($instance[$image_name_2])){echo esc_url( $instance[$image_name_2] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat medify_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name_2) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>" value="<?php echo esc_attr( $image_2 ); ?>">
            <a href="#" class="button medify_media_upload"><?php esc_html_e('Upload', 'medify-core'); ?></a>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $padding_top ) ); ?>"><?php echo esc_html__('Padding Top: ', 'medify-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $padding_bottom ) ); ?>"><?php echo esc_html__('Padding Bottom: ', 'medify-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>"><?php echo esc_html__('Sub Title:', 'medify-core')?></label> 
            <textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_sub_title ) ); ?>" row="2"><?php echo Medify_Theme_Helper::render_html($text_sub); ?></textarea>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>"><?php echo esc_html__('Title:', 'medify-core')?></label> 
            <textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_name ) ); ?>" row="2"><?php echo Medify_Theme_Helper::render_html($text); ?></textarea>
        </p>            

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $title_name ) ); ?>"><?php echo esc_html__('Button title:', 'medify-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'Button link:', 'medify-core' ); ?></label> 
