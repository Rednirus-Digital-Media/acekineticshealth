...
class WglThemeHelper{
    ...
    public function render_post_share ($show_share) {
        ...
        ?>
            ...
            <div class="share_social-wpapper">					
                ...
                <?php
                    if (isset($img_url[0]) && strlen($img_url[0]) > 0) {
    ...
    public static function render_social_shares(){
        $description = esc_html__('Socials', 'medify-core');

        $facebook    = Medify_Theme_Helper::options_compare('soc_icon_facebook', 'mb_customize_soc_shares', 'on');
        $twitter     =  Medify_Theme_Helper::options_compare('soc_icon_twitter', 'mb_customize_soc_shares', 'on');
        $linkedin    = Medify_Theme_Helper::options_compare('soc_icon_linkedin', 'mb_customize_soc_shares', 'on');
        $pinterest   = Medify_Theme_Helper::options_compare('soc_icon_pinterest', 'mb_customize_soc_shares', 'on');
        $tumblr      = Medify_Theme_Helper::options_compare('soc_icon_tumblr', 'mb_customize_soc_shares', 'on');

        $fixed   = Medify_Theme_Helper::options_compare('soc_icon_position', 'mb_customize_soc_shares', 'on');
        $offset = Medify_Theme_Helper::get_option('soc_icon_offset');
        ...
        $share = Medify_Theme_Helper::options_compare('soc_icon_style', 'mb_customize_soc_shares', 'on');
            ...
            <div class="share_social-wpapper">
                ...
                ?>
                    <div class="share_social-desc">
                        ...
                        <?php
                            echo apply_filters('medify_desc_socials', $description);
                        ?>                                
                        </div>
                ...
                <ul>
                    ...
                    $custom_share = Medify_Theme_Helper::get_option('add_custom_share');
                    if(isset($custom_share) && !empty($custom_share)){
                        for ($i = 1; $i <= 10; $i++) {
                            ${'custom_share_'.$i} = Medify_Theme_Helper::get_option('select_custom_share_text-'.$i);
                            ${'custom_share_icons'.$i} = Medify_Theme_Helper::get_option('select_custom_share_icons-'.$i);
