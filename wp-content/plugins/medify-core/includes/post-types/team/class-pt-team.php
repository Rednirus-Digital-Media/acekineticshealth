...
class Team {
    ...
    public function __construct() {
        ...
        $this->name = __( 'Team', 'medify-core' );
        $this->singular_name = __( 'Member', 'medify-core' );
        $this->plural_name = __( 'Members', 'medify-core' );

        $this->slug = Medify_Theme_Helper::get_option('team_slug') != '' ? Medify_Theme_Helper::get_option('team_slug') : 'team';
...
function render_wgl_team_item ( $single_member, $item_atts, $team_image_dims ) {
    ...
    if (!(bool)$single_member) {
        ...
        $compile .= '<div class="team-single_wrapper" '.(!empty($info_bg_url) ? 'style="background-image:url('.$info_bg_url.')"' : '').'>';
...
function render_wgl_team ($atts) {
    ...
    switch ($posts_per_line) {
        ...
        case '4':
        case '5': $team_image_dims = array('width' => '364', 'height' => '470');  break;
    }

    list($query_args) = Medify_Loop_Settings::buildQuery($item_atts);
