...
$sb = Medify_Theme_Helper::render_sidebars('portfolio_single');
...
?>
<div class="wgl-container single_portfolio<?php echo apply_filters('medify_container_class', $container_class); ?>">
	<div class="row<?php echo apply_filters('medify_row_class', $row_class); ?>">
		<div id='main-content' class="wgl_col-<?php echo apply_filters('medify_column_class', $column); ?>">
			...
			$related_switch = Medify_Theme_Helper::get_option('portfolio_related_switch');
			...
			if ( (bool)$related_switch && class_exists('Medify_Core') && class_exists('Elementor\Plugin')) :
				$mb_pf_cat_r = array();
				
				$mb_pf_carousel_r = Medify_Theme_Helper::options_compare('pf_carousel_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_title_r = Medify_Theme_Helper::options_compare('pf_title_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_column_r = Medify_Theme_Helper::options_compare('pf_column_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_number_r = Medify_Theme_Helper::options_compare('pf_number_r', 'mb_portfolio_related_switch', 'on');
				...
				if($featured_render->post_count > 0){
					echo '<div class="related_portfolio">';
						if(!empty($mb_pf_title_r)){
							echo '<div class="medify_module_title"><h4>' . esc_html($mb_pf_title_r) . '</h4></div>';
						}
						echo $featured_post;
					echo '</div>';
				}
