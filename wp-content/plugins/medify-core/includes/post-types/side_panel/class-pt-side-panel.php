...
class SidePanel {
    ...
    public function __construct() {
        ...
        $this->name = __( 'Side Panel', 'medify-core' );
        $this->slug = 'side-panel';
        $this->plural_name = __( 'Side Panel', 'medify-core' );
    ...
    public function register() {
        $labels = array(
            ...
            'add_new'               => sprintf( __('Add New %s', 'medify-core' ), $this->name ),
            'add_new_item'          => sprintf( __('Add New %s', 'medify-core' ), $this->name ),
            'edit_item'             => sprintf( __('Edit %s', 'medify-core'), $this->name ),
            'new_item'              => sprintf( __('New %s', 'medify-core'), $this->name ),
    ...
    public function wrapper_single_side_panel_open(){
        $pos = Medify_Theme_Helper::options_compare('side_panel_position', 'mb_customize_side_panel', 'custom');

        $content_type = Medify_Theme_Helper::options_compare('side_panel_content_type','mb_customize_side_panel','custom');
        ...
        echo '<div class="side-panel_overlay"></div>';
            ...
            //Get options   
        $side_panel_spacing = Medify_Theme_Helper::options_compare('side_panel_spacing','mb_customize_side_panel','custom');
    ...
    public function side_panel_style(){         
        ...
        $bg = Medify_Theme_Helper::get_option('side_panel_bg');
        $bg = !empty($bg['rgba']) ? $bg['rgba'] : '';

        $color = Medify_Theme_Helper::get_option('side_panel_text_color');
        $color = !empty($color['rgba']) ? $color['rgba'] : '';

        $width = Medify_Theme_Helper::get_option('side_panel_width');
        $width = !empty($width['width']) ? $width['width'] : '';

        $align = Medify_Theme_Helper::options_compare('side_panel_text_alignment', 'mb_customize_side_panel', 'custom'); 
        $style = '';
