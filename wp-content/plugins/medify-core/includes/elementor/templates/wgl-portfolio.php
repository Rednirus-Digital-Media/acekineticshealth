...
class WglPortfolio{
    ...
    public function render( $atts ){
        ...
        // * Add custom id
        $params['item_id'] = $item_id = uniqid('portfolio_module_');
        ...
        if ( $navigation == 'pagination' ) {
            ...
            $out .= \Medify_Theme_Helper::pagination('10', $query_results, $nav_align);
        }
    ...
    private function post_content ($params){
        ...
        $content = \Medify_Theme_Helper::modifier_character($content, $chars_count, "");
    ...
    public function wgl_portfolio_item($params, $class, $count, $grid_gap){
        ...
        switch ($click_area) {
            case 'popup':
                $link = "<a href='" . $wp_get_attachment_url . "' class='portfolio_link swipebox' data-elementor-open-lightbox='yes' data-elementor-lightbox-slideshow='".esc_attr($params['item_id'])."'><i class='portfolio_link-icon flaticon-search-4'></i></a>";
                break;
            case 'single':
        ...
        if((bool)$show_portfolio_title){
            ...
            $tag_attr = (bool)$single_link_title ? 'href="'.$title_link.'" target="_blank"' : '';
        ...
        if ((bool)$gallery_mode) {
            $out .= '<div class="wgl-portfolio-item_gallery-icon"><i class="flaticon-add"></i></div>';
        }
    ...
    private function single_post_date(){
        ...
        if (rwmb_meta('mb_portfolio_single_meta_date') == 'default') {
            $date = \Medify_Theme_Helper::get_option('portfolio_single_meta_date');
        } else {
    ...
    private function single_post_likes(){
        $show_likes = \Medify_Theme_Helper::get_option('portfolio_single_meta_likes');
    ...
    private function single_post_author(){
        $author = \Medify_Theme_Helper::get_option('portfolio_single_meta_author');
        if( !empty($author) ) {
    ...
    private function single_post_comments(){
        $comments = \Medify_Theme_Helper::get_option('portfolio_single_meta_comments');
    ...
    private function single_post_cats(){
        ...
        if (rwmb_meta('mb_portfolio_single_meta_categories') == 'default') {
            $cat = \Medify_Theme_Helper::get_option('portfolio_single_meta_categories');
        } else {
    ...
    public function wgl_portfolio_single_fw(){
        ...
        if (class_exists( 'RWMB_Loader' )) {
            ...
            $mb_meta_info = \Medify_Theme_Helper::get_option('portfolio_single_meta');
        }

        $single_post_type = \Medify_Theme_Helper::options_compare('portfolio_single_type_layout','mb_portfolio_post_conditional','custom');
        $single_post_align = \Medify_Theme_Helper::options_compare('portfolio_single_align','mb_portfolio_post_conditional','custom');
        $portfolio_single_padding = \Medify_Theme_Helper::options_compare('portfolio_single_padding','mb_portfolio_post_conditional','custom');

        $portfolio_parallax = \Medify_Theme_Helper::options_compare('portfolio_parallax', 'mb_portfolio_post_conditional', 'custom');
        $portfolio_parallax_speed = \Medify_Theme_Helper::options_compare('portfolio_parallax_speed', 'mb_portfolio_post_conditional', 'custom');
    ...
    public function wgl_portfolio_single_item($parameters, $item_class = ''){
        ...
        if (class_exists( 'RWMB_Loader' )) {
            ...
            if (rwmb_meta('mb_portfolio_above_content_cats') == 'default') {
                $mb_cats_under = \Medify_Theme_Helper::get_option('portfolio_above_content_cats');
            } else {
                $mb_cats_under = rwmb_meta('mb_portfolio_above_content_cats');
            }
            if (rwmb_meta('mb_portfolio_above_content_share') == 'default') {
                $mb_soc_under = \Medify_Theme_Helper::get_option('portfolio_above_content_share');
            } else {
        ...
        if ($mb_soc_under == 1 || $mb_soc_under == 'yes') {
            echo '<div class="single_info-share_social-wpapper">';
                echo '<span class="share_title">'.esc_html__('Share:','medify-core').'</span>';
        ...
        if ((bool)$mb_download) {
            echo '<div class="portfolio_info_item-download">';
                echo '<div class="medify_module_button wgl_button wgl_button-m acenter wgl_button-full"><a class="wgl_button_link" href="'.esc_url($mb_download_link).'">'.esc_html($mb_download_text).'</a></div>';
            echo '</div>';
        }
    ...
    static public function getImgUrl ($params, $wp_get_attachment_url, $crop, $count, $grid_gap) {
        $masonry_gap = '';
        ...
        if (strlen($wp_get_attachment_url)) {
            if ($params['portfolio_layout'] == 'masonry2') {
                ...
                $wgl_featured_image_url = aq_resize($wp_get_attachment_url, "1000", "540", $crop, true, true);
            }else{
                switch ($params['posts_per_row']) {
    ...
    public function getCategories($params, $query){
        ...
        $out = '<a href="#" data-filter=".item" class="active">'.esc_html__('All','medify-core').'<span class="number_filter"></span></a>';
        foreach ($cats as $cat) {
            if($cat->count > 0){
