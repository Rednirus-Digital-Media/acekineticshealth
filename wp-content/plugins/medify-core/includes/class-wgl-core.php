...
class Medify_Core {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Medify_Core_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function wgl_ajax_init (){
	    ...
	    switch ($post_type) {
	    	...
	    	case 'product':
				...
				if ( 1 == $total ) {
					$out .= esc_html__( 'Showing the single result', 'medify-core' );
				} elseif ( $total <= $per_page || -1 == $per_page ) {
					$out .= sprintf( esc_html__( 'Showing all %d results', 'medify-core' ), $total );
				} else {
					$out .= sprintf( _x( 'Showing <strong>%1$d&ndash;%2$d</strong> of %3$d results', '%1$d = first, %2$d = last, %3$d = total', 'medify-core' ), $first, $last, $total );
				}
	...
	public function wgl_mega_menu_load_ajax (){
		...
		$out = '';
	    list($query_args) = Medify_Loop_Settings::buildQuery($_POST);
	...
	public function wgl_cache_query($args = array()){
		...
		$custom_query = wp_cache_get( $cache_key, 'medify_theme' );
		if ( false === $custom_query ) {
			$custom_query = new WP_Query( $args );
		    if ( ! is_wp_error( $custom_query ) && $custom_query->have_posts() ) {

		    	wp_cache_set( $cache_key, $custom_query, 'medify_theme' );
		    }
		}
	  	return $custom_query;
	}
	 ...
	 *
	 * - Medify_Core_Loader. Orchestrates the hooks of the plugin.
	 * - Medify_Core_i18n. Defines internationalization functionality.
	 * - Medify_Core_Admin. Defines all hooks for the admin area.
	 * - Medify_Core_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		...
		$this->loader = new Medify_Core_Loader();
	 ...
	 *
	 * Uses the Medify_Core_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Medify_Core_i18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new Medify_Core_Admin( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Medify_Core_Public( $this->get_plugin_name(), $this->get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    Medify_Core_Loader    Orchestrates the hooks of the plugin.
	 */
