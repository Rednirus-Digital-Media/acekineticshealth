    ...
    class ReduxFramework_custom_preset{
        ...
        public function render() {
            ...
            <div class="wgl_preset_notice">
                <?php
                    esc_html_e('Notice', 'medify-core');
                    echo '<ul>';
                        echo '<li>';
                            echo '<div class="notice_h">';
                            echo '</div>';
                            esc_html_e('We will reset all options inside header builder', 'medify-core');
                        echo '</li>';
                        echo '<li>';
                            esc_html_e('You can modify your header using header builder after choosing header builder template. Click "Save Preset", to save the header builder layout', 'medify-core');
                        echo '</li>';
                    echo '<ul>';
                ?>
            </div>
            <div id="wgl_custom_preset" class="wgl_custom_preset-wrapper">
            <div id="redux-add-preset-action"><input type="submit" id="redux-add-preset" value="<?php esc_html_e( 'Add Preset', 'medify-core' ) ?>"><i class="fa fa-plus"></i></div>
            ...
            <div id="redux-save-preset-action" style="display:none;">
                <div class="wrap-save-preset">
                    ...
                    <input type="submit" id="redux-save-preset" class="button-primary" value="<?php esc_html_e( 'Save', 'medify-core' ) ?>">
                </div>
            ...
            if ( isset( $this->field['options'] ) ) {
                if(!empty($this->field['options'])){
                    echo "<h4>".__('Saved Templates', 'medify-core')."</h4>";
                }
                    ...
                    if ( $selected ) {
                        if ( empty( $v['presets'] ) ) {
                            ...
                            foreach ($v['presets'] as $pk => $pv ) {
                                if ( ! isset( $this->parent->options[ $pk ] ) ) {
                                    $selected = false;
                                }
                                if ( ! $selected ) {
                                    $this->value = "";
                    ...
                    echo '<div class="wrapper-btn-preset">';
                        ...
                        if ( $selected ) {
                            echo '<div id="selected-btn-preset" class="btn-preset">';
                                echo '<input type="submit" class="selected-btn_custom btn-primary redux-update-preset" value="'.apply_filters( "wgl-changed-text-save-preset-{$this->parent->args['opt_name']}", __( 'Save Preset', 'medify-core' ) ).'">';

                            echo '</div>';
                        }

                        if(!$v['default'] && !array_key_exists($k, $list_defualt_preset)){
                            echo '<input type="submit" class="selected-btn_custom btn-primary redux-rename-preset" value="'.apply_filters( "wgl-changed-text-rename-preset-{$this->parent->args['opt_name']}", __( 'Rename', 'medify-core' ) ).'">';

                             echo '<div class="delete-preset">';
                                echo '<a href="#" title="'.apply_filters( "wgl-changed-text-delete-preset-{$this->parent->args['opt_name']}", __( 'Delete Preset', 'medify-core' ) ).'" class="redux-delete-preset">';
                        ...
                        if(!$v['default'] && array_key_exists($k, $list_defualt_preset)){
                        	if($selected){
	                            echo '<div class="delete-preset">';
	                                echo '<a href="#" title="'.apply_filters( "wgl-changed-text-reset-preset-{$this->parent->args['opt_name']}", __( 'Reset', 'medify-core' ) ).'" class="redux-delete-preset reset-default">';
        ...
        public function enqueue() {
            ...
            $vars = array(
                'delete'   => __( 'Delete Preset?', 'medify-core' ),
                'reset'   => __( 'Reset to default?', 'medify-core' ),
                'taken_name'   => __( 'Sorry this name is already exists', 'medify-core' ),
                'empty_name'   => __( 'Empty name', 'medify-core' ),
            );
